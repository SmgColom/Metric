...
export async function getServerSideProps({ params, query }) {
  ...
  try {
    ...
    const allScores = allScoresRaw.map((r) => {
      const laps = extractLaps(r);
      const checkpoints = extractCheckpoints(r);
      const split = buildSplitLines({ laps, checkpoints });
      ...
      return {
        ...
        team: fixText(r?.team),
        sex: (r?.sex ?? "").toString().trim().toUpperCase(),
    ...
    // ✅ decide si el evento (en general) tiene laps o checkpoints
    const hasAnyLaps = allScores.some((r) => r.splitMode === "laps" && r.splitLines.length);
    const hasAnyCps = allScores.some((r) => r.splitMode === "checkpoints" && r.splitLines.length);

    // prioridad: laps > checkpoints
    const splitMeta = hasAnyLaps
      ? { enabled: true, mode: "laps", header: "Vueltas" }
      : hasAnyCps
      ? { enabled: true, mode: "checkpoints", header: "Checkpoints" }
      : { enabled: false, mode: null, header: null };

    // ✅ Posición por género (calculada) usando sex + net_score
    const genderRankMap = new Map(); // SEX::bib => rank
    ...
    // 10) Enriquecer filas
    const rowsWithRanks = rows.map((r) => {
      const bib = String(r?.bib ?? "");
      const sex = (r?.sex ?? "").toString().trim().toUpperCase();

      // ✅ solo mostramos splits del modo ganador del evento (laps o cps)
      const effectiveLines =
        splitMeta.enabled && r.splitMode === splitMeta.mode ? r.splitLines : [];
      ...
      return {
        ...
        splitLines: effectiveLines,
      };
    });

    const data = {
      ...base,
      results: rowsWithRanks,
      resultsMeta: {
        total,
        page: safePage,
        pageSize,
        ...
        splitHeader: splitMeta.header, // "Vueltas" o "Checkpoints"
      },
    };
